...
output_fp <- paste("C:/Users/Caleb Vogt/Box/CV_Shared_Analyses/7_LID_2020/RFID_DATA/RFID_analysis_v3/1_GMM_RData_Files")
...
## READ IN DATA
filenames <- list.files(wd, pattern = "*GMM_GBI_DATA.csv")
...
for(aa in 1:length(myfiles)){
  ...
  col_ids <- colnames(df2[,13:ncol(df2)])
  ...
  for(bb in col_ids[1:length(col_ids)]) {
    df3 <- df2 %>% 
      ...
      mutate(Name = bb) %>% 
      select(Day, Zone, Location, Name, Start, End, Field_Time_START, Field_Time_STOP, m_sum, f_sum, mf_sum)
      
    ## ADD RELEVANT METADATA INFORMATION. 
    df4 <- merge(df3, meta_short, by.x = "Name", by.y = "name")
    df5 <- df4 %>% 
      relocate(trial, 
               paddock, Day, Zone, Location, 
               strain, sex, Name, code, family_group, 
               Start, End, Field_Time_START, 
               Field_Time_STOP, m_sum, f_sum, mf_sum)
